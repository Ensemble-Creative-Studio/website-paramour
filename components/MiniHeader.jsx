  ...
  export default function MiniHeader({ projectData }) {
    ...
    return (
      <header className="fixed h-28 px-12 md:px-10 left-0 flex justify-between w-full top-10 md:top-0 z-20 items-center">
        ...
        <div className="hidden md:flex items-center gap-3 md:flex-1 md:justify-center">
        ...
        <div className="flex gap-3 md:flex-1 md:justify-end md:items-center">
          <div
            className='slideCount -z-20 relative menuFooter hidden md:block'  id="slideCountDiv"
          ></div>
  <div className="itemFooter grey cursor-pointer hidden md:block" onClick={handleViewAllClick}>VIEW ALL</div>
          <span
        ...
        <div
          className={`${isVisible ? "transition-opacity-active " : "transition-opacity"} ${
        ...
        <ViewAll projectData={projectData} isVisible={isViewAllVisible} setIsVisible={setIsViewAllVisible} />
